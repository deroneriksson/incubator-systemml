...
public class ScalarScalarArithmeticCPInstruction extends ArithmeticBinaryCPInstruction
{
	...
	public void processInstruction(ExecutionContext ec) throws DMLRuntimeException{
		...
		{
			System.out.println("$$$$$$" + ec.getScriptType());
			String val1 = null;
			if (so1 instanceof BooleanObject) {
				val1 = ((BooleanObject) so1).getLanguageSpecificBooleanStringValue(ec.getScriptType());
			} else {
				val1 = so1.getStringValue();
			}
			String val2 = null;
			if (so2 instanceof BooleanObject) {
				val2 = ((BooleanObject) so2).getLanguageSpecificBooleanStringValue(ec.getScriptType());
			} else {
