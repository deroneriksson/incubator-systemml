...
//	}
	...
	public static CSRPointer columnMajorDenseToRowMajorSparse(GPUContext gCtx, cusparseHandle cusparseHandle, Pointer densePtr, int rows, int cols) throws DMLRuntimeException {
		...
		long t2=0;
		if (RuntimePlatform.statistics) t2 = System.nanoTime();
		cudaMemcpy(Pointer.to(nnzC), nnzTotalDevHostPtr, getIntSizeOf(1), cudaMemcpyDeviceToHost);
		if (RuntimePlatform.statistics) GPUStatistics.cudaFromDevTime.addAndGet(System.nanoTime() - t2);
		if (RuntimePlatform.statistics) GPUStatistics.cudaFromDevCount.addAndGet(1);
	...
	public void denseToSparse() throws DMLRuntimeException {
		...
		if (RuntimePlatform.statistics) GPUStatistics.cudaDenseToSparseTime.addAndGet(System.nanoTime() - t0);
		if (RuntimePlatform.statistics) GPUStatistics.cudaDenseToSparseCount.addAndGet(1);
	}
	...
	public void sparseToDense(String instructionName) throws DMLRuntimeException {
		...
		long start=0, end=0;
		if (RuntimePlatform.statistics) start = System.nanoTime();
		...
		if (RuntimePlatform.statistics) GPUStatistics.cudaSparseToDenseTime.addAndGet(end - start);
		if (RuntimePlatform.statistics) GPUStatistics.cudaSparseToDenseCount.addAndGet(1);
	}
...
//				values = new double[0];
				...
				if (block instanceof SparseBlockCSR){
					...
					if (RuntimePlatform.statistics) GPUStatistics.cudaSparseConversionTime.addAndGet(System.nanoTime() - t0);
					if (RuntimePlatform.statistics) GPUStatistics.cudaSparseConversionCount.incrementAndGet();
		...
		if (RuntimePlatform.statistics) GPUStatistics.cudaToDevTime.addAndGet(System.nanoTime()-start);
		if (RuntimePlatform.statistics) GPUStatistics.cudaToDevCount.addAndGet(1);
	}
	...
	protected void copyFromDeviceToHost() throws DMLRuntimeException {
		...
		if(getJcudaDenseMatrixPtr() != null) {
			...
			if (RuntimePlatform.statistics) GPUStatistics.cudaFromDevTime.addAndGet(System.nanoTime()-start);
			if (RuntimePlatform.statistics) GPUStatistics.cudaFromDevCount.addAndGet(1);
		}
