...
public class MatrixBlock extends MatrixValue implements CacheBlock, Externalizable
{
			...
			throws RuntimeException 
	{
		...
		if(denseBlock == null || denseBlock.length < limit) {
			long start = DISPLAY_STATISTICS && RuntimePlatform.statistics ? System.nanoTime() : 0;
			denseBlock = new double[(int)limit];
			Statistics.allocateDoubleArrTime += DISPLAY_STATISTICS && RuntimePlatform.statistics ? (System.nanoTime() - start) : 0;
		}
		...
		throws DMLRuntimeException
	{
		...
		Statistics.examSparsityTime += DISPLAY_STATISTICS && RuntimePlatform.statistics ? (System.nanoTime() - start) : 0;
	}
	...
	public void recomputeNonZeros()
	{
		...
		else if( !sparse && denseBlock!=null ) //DENSE (max int)
		{
			...
			for(int i=0; i<limit; i++)
				nnz += (a[i]!=0) ? 1 : 0;
			nonZeros = nnz;
			Statistics.recomputeNNZTime += DISPLAY_STATISTICS && RuntimePlatform.statistics ? (System.nanoTime() - start) : 0;
		}
	}
