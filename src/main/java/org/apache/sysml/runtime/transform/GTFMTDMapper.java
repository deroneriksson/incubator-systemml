...
public class GTFMTDMapper implements Mapper<LongWritable, Text, IntWritable, DistinctValue>{
	...
	public void configure(JobConf job) {
		String[] parts = job.get(MRConfigurationNames.MR_TASK_ID).split("_");
		if ( parts[0].equalsIgnoreCase("task")) {
			...
			throw new RuntimeException("Unrecognized format for taskID: " + job.get(MRConfigurationNames.MR_TASK_ID));
		}
